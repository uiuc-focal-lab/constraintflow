...
func backsubs_lower(PolyExp e, Neuron n) = (e.traverse(backward, -n[layer], true, replace_lower){e<=n}).map(simplify_lower);
func backsubs_upper(PolyExp e, Neuron n) = (e.traverse(backward, -n[layer], true, replace_upper){e>=n}).map(simplify_upper);
...
flow(forward, n[layer], true, deeppoly)
